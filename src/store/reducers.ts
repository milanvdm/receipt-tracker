...
const ReceiptTrackerReducer = (state = initialState, action: ReceiptTrackerActionType): ReceiptTrackerState => {
    switch (action.type) {
        case ADD_RECEIPT: {
            const id = uuid();
        ...
        case UPDATE_CATEGORY: {
            return {
                receipts: state.receipts.setIn([action.payload.receiptId, 'category'], action.payload.category),
            };
        }
        case ADD_EXPENSE: {
            const id = uuid();
